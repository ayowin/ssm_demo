<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>index</title>
</head>
<body>
	<h2>Hello World! This is index.jsp!</h2>

	<br />

	<a href="springmvc.jsp" style="font-size: 24px;">静态文件：springmvc.jsp</a>

	<br />

	<a href="springmvc" style="font-size: 24px;">MVC测试：springmvc</a>

	<br />

	<a href="insert" style="font-size: 24px;">增：insert</a>

	<br />

	<a href="update" style="font-size: 24px;">改：update</a>

	<br />

	<a href="select" style="font-size: 24px;">查：select</a>

	<br />

	<a href="delete" style="font-size: 24px;">删：delete</a>
	
	<br />

	<a href="FreeMarkerTest" style="font-size: 24px;">FreeMarkerTest</a>

</body>
</html>